...
#define ONE_WIRE_BUS A3 // и настраиваем  пин 9 как шину подключения датчиков DS18B20
...
String at = "";
String SMS_phone = "+375295912507"; // куда шлем СМС
String call_phone = "375291853337"; // телефон хозяина без плюса
...
float Vstart = 13.2;     // поорог распознавания момента запуска по напряжению
  ...
  if (m590.available()) { // если что-то пришло от модема 
    ...
    if (at.indexOf("RING") > -1) {
      ...
      Serial.println("Incoming call is cleared"),WarmUpTimer = 60, start = true;
         ...
         m590.print("#M5-90-45-78-12-35#M590+DS18B20"); // индивидуальный номер для народмона
         m590.print("\n#Temp1#");   // название первого датчиака для народмона
         m590.print(tempds0);       // значение первого датчиака для народмона
         m590.print("\n#Temp2#");   // название второго датчиака для народмона
         m590.print(tempds1);       // значение второго датчиака для народмона
         m590.print("\n#Vbat#");    // название параметра напряжения для народмона
         m590.print(Vbat);          // напряжение АКБ для отображения на народмоне
...
void detection(){ // условия проверяемые каждые 10 сек  
    ...
    if (tempds0==-127) tempds0 = 54.99;  
    tempds1 = sensors.getTempCByIndex(1);
    if (tempds1==-127) tempds1 = 54.99;
...
//    if (tempds2==-127) tempds2 = 54.99;
  ...
  Serial.print(" || WarmUpTimer ="), Serial.println (WarmUpTimer);
    ...
    if (modem == 0 && SMS_send == true) {  // если фаг SMS_send равен 1 высылаем отчет по СМС
        ...
        m590.println("AT+CMGS=\"" + SMS_phone + "\"");
        delay(100);
...
int StarterTime = 1000;  // переменная хранения времени работы стартера (1 сек. для первой попытки)  
 while (Vbat > 10.00 && digitalRead(Feedback_Pin) == LOW && digitalRead(STOP_Pin) == LOW && count > 0) 
    ...
    count--; // уменьшаем на еденицу число оставшихся потыток запуска
        if (Vbat > Vstart) // если АКБ заряжается напряжением выше Vstart
        { 
      ...
      //  delay (500), SMS_send = true; // если нет запуска отправим СМС
        }
  
    }
  
  //  heating = true, Serial.println (" Vbat < 10.00 V || Feedback - HIGH || STOP_Pin - HIGH. heating = true");
