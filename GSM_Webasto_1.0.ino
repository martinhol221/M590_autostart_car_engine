...
/*  ----------------------------------------- НАЗНАЧАЕМ ВЫВДЫ --------------------------------------------------------------------   */
#define ON_Pin 9                    // на реле вебасты
#define BAT_Pin A0                  // на батарею, через делитель напряжения 39кОм / 11 кОм
#define Feedback_Pin A3             // на силовой провод замка зажигания для обратной связи
/*  ----------------------------------------- ИНДИВИДУАЛЬНЫЕ НАСТРОЙКИ !!!---------------------------------------------------------   */
String call_phone = "375000000000"; // телефон входящего вызова
String SMS_phone = "+375000000000"; // телефон куда отправляем СМС
String MAC = "59-01-AA-00-00-56";   // МАС-Адрес устройства для индентификации на сервере narodmon.ru (придумать свое 59-01-XX-XX-XX-XX-XX)
...
//  if (digitalRead(STOP_Pin) == HIGH) SMS_report = true;  // включаем народмон при нажатой педали тормоза при подаче питания 
  Serial.print("Starting M590 Webasto 31.10.2017, SMS_report =  "), Serial.println(SMS_report);
...
void loop() {
  if (m590.available()) { // если что-то пришло от модема 
    ...
    if (at.indexOf("RING") > -1) {
        ...
        if (at.indexOf(call_phone) > -1) delay(50), m590.println("ATH0"), webasto_ON();      // сбрасываем вызов, включаем вебасту
...
if (heating == true && digitalRead(Feedback_Pin) == HIGH) webasto_OFF();
...
void detection(){                           // услови проверяемые каждые 10 сек  
    ...
    if (SMS_send == true && SMS_report == true) { SMS_send = false;               // если фаг SMS_send равен 1 высылаем отчет по СМС
        ...
        m590.print(""+SENS+"  v 31.10.2017 ");
        m590.print("\n Temp.Dvig: "),  m590.print(TempDS0);
        m590.print("\n Temp.Salon: "), m590.print(TempDS1);
        if (heating= true)     m590.print("\n Webasto ON");
        m590.print("\n Vbat: "),       m590.print(Vbat),  m590.print("V.");
        m590.print((char)26);                   }
     
    if (WarmUpTimer == (SMS_time * 6)) WarmUpTimer--, SMS_send = true;   
    
    if (WarmUpTimer > 0 )                               WarmUpTimer--;  
    if (heating == true && WarmUpTimer <1)              webasto_OFF(); 
...
void webasto_ON() {                                       // программа включения прогрева
    WarmUpTimer = 30;                                     // прогрев по умолчанию 5 минут
