...
int k = 0;
int interval = 6;                  // интервал отправки данных на народмон 20 сек после старта
...
void detection(){                           // услови проверяемые каждые 60 сек  
  ...
  Serial.print(" || WarmUpTimer ="), Serial.println (WarmUpTimer);
    ...
    if (interval <1 )                                   interval = 5, m590.println ("AT+XISP=0"), delay(100); // выходим в интернет
//  if (interval == 4 )                                 delay (100), m590.println("AT+TCPCLOSE=0");
}             
 
void enginestart() {                                                   // программа запуска двигателя
if (Timer_time > 5 && Timer_time <60) WarmUpTimer = Timer_time;        // если таймер в пределах от 5 до 60 мин переводя минуты в десятки секунд
